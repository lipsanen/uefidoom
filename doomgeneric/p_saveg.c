...
static byte saveg_read8(void)
{
    ...
    if (d_fread(&result, 1, 1, save_stream) < 1)
    {
        if (!savegame_error)
        {
            d_printf( "saveg_read8: Unexpected end of file while "
                            "reading save game\n");
...
{
    if (d_fwrite(&value, 1, 1, save_stream) < 1)
    {
        if (!savegame_error)
        {
            d_printf( "saveg_write8: Error while writing save game\n");
