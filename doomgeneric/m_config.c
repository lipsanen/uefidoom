...
static default_t *SearchCollection(default_collection_t *collection, char *name)
{
    ...
    {
        if (!d_strcmp(name, collection->defaults[i].name))
        {
...
static int ParseIntParameter(char *strparm)
{
    ...
    if (strparm[0] == '0' && strparm[1] == 'x')
        d_sscanf(strparm+2, "%x", &parm);
    else
        d_sscanf(strparm, "%i", &parm);
...
{
    size_t len = d_strlen(src) + 1;
    char* output =Z_Malloc(len, PU_STATIC, NULL);
    d_memcpy(output, src, len);
    return output;
}
...
static void SetVariable(default_t *def, char *value)
{
    ...
    {
        case DEFAULT_STRING:
            // This seems stupid, why is it just leaking memory???
...
static void LoadDefaultCollection(default_collection_t *collection)
{
    ...
    while (!feof(f))
    {
        ...
        if (d_strlen(strparm) >= 2
         && strparm[0] == '"' && strparm[d_strlen(strparm) - 1] == '"')
        {
            strparm[d_strlen(strparm) - 1] = '\0';
...
void M_SetConfigDir(char *dir)
{
    ...
    if (d_strcmp(configdir, "") != 0)
    {
        d_printf("Using %s for configuration and saves\n", configdir);
    }
...
char *M_GetSaveGameDir(char *iwadname)
{
    ...
    else
    {
        ...
        d_printf ("Using %s for savegames\n", savegamedir);
    }
