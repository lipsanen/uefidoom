...
void T_VerticalDoor(doom_data_t *doom, vldoor_t *door)
{
	...
	case 0:
		...
		if (!--door->topcountdown)
		{
			...
			case vld_blazeRaise:
				...
				S_StartSound(doom, &door->sector->soundorg, sfx_bdcls);
				break;

			case vld_normal:
				door->direction = -1; // time to go back down
				S_StartSound(doom, &door->sector->soundorg, sfx_dorcls);
				break;

			case vld_close30ThenOpen:
				door->direction = 1;
				S_StartSound(doom, &door->sector->soundorg, sfx_doropn);
				break;
	...
	case -1:
		// DOWN
		res = T_MovePlane(doom, door->sector,
						  door->speed,
					...
					mobj_t *thing)
{
	...
	return EV_DoDoor(doom, line, type);
}
			  ...
			  vldoor_e type)
{
	...
	while ((secnum = P_FindSectorFromLineTag(doom, line, secnum)) >= 0)
	{
		...
		{
		case vld_blazeClose:
			door->topheight = P_FindLowestCeilingSurrounding(doom, sec);
		...
		case vld_blazeOpen:
			...
			if (door->topheight != sec->ceilingheight)
				S_StartSound(doom, &door->sector->soundorg, sfx_bdopn);
			break;
					 ...
					 mobj_t *thing)
{
		...
		case 117:
			...
			else
			{
				...
				else
				{
					...
					d_printf("EV_VerticalDoor: Tried to close "
	...
	case 118: // BLAZING DOOR OPEN
		S_StartSound(doom, &sec->soundorg, sfx_bdopn);
		break;

	case 1: // NORMAL DOOR SOUND
	case 31:
		S_StartSound(doom, &sec->soundorg, sfx_doropn);
		break;
