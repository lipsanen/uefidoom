...
void DG_Init()
{
	d_memset(s_KeyQueue, 0, KEYQUEUE_SIZE * sizeof(unsigned short));
    ...
    d_memset(&attr, 0, sizeof(XSetWindowAttributes));
...
void DG_SetWindowTitle(const char * title)
{
    if (s_Window)
    {
        XChangeProperty(s_Display, s_Window, XA_WM_NAME, XA_STRING, 8, PropModeReplace, title, d_strlen(title));
    }
}
