...
void Z_Init(struct doom_data_t_* doom)
{
    ...
    mainzone = (memzone_t *)I_ZoneBase(doom, &size);
    mainzone->size = size;
                ...
                int hightag)
{
    ...
    d_printf("tag range: %i to %i\n",
             lowtag, hightag);

    for (block = mainzone->blocklist.next;; block = block->next)
    {
        if (block->tag >= lowtag && block->tag <= hightag)
            d_printf("block:%p    size:%7i    user:%p    tag:%3i\n",
        ...
        if (block->tag == PU_FREE && block->next->tag == PU_FREE)
            d_printf("ERROR: two consecutive free blocks\n");
    }
}
...
void Z_FileDumpHeap(FILE *f)
{
    ...
    d_fprintf(f, "zone size: %i  location: %p\n", mainzone->size, mainzone);

    for (block = mainzone->blocklist.next;; block = block->next)
    {
        d_fprintf(f, "block:%p    size:%7i    user:%p    tag:%3i\n",
                  block, block->size, block->user, block->tag);
        ...
        if ((byte *)block + block->size != (byte *)block->next)
            d_fprintf(f, "ERROR: block size does not touch the next block\n");

        if (block->next->prev != block)
            d_fprintf(f, "ERROR: next block doesn't have proper back link\n");

        if (block->tag == PU_FREE && block->next->tag == PU_FREE)
            d_fprintf(f, "ERROR: two consecutive free blocks\n");
    }
}
