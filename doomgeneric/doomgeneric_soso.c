...
void disableRawMode()
{
  //d_printf("returning original termios\n");
...
void DG_Init()
{
    ...
    if (FrameBufferFd >= 0)
    {
        ...
        {
            d_printf("FrameBuffer mmap success\n");
        }
        else
        {
            d_printf("FrameBuffermmap failed\n");
        }
    }
    else
    {
        d_printf("Opening FrameBuffer device failed!\n");
    }
    ...
    if (argPosX > 0)
    {
        d_sscanf(myargv[argPosX + 1], "%d", &s_PositionX);
    }

    argPosY = M_CheckParmWithArgs("-posy", 1);
    if (argPosY > 0)
    {
        d_sscanf(myargv[argPosY + 1], "%d", &s_PositionY);
    }
}
...
void DG_DrawFrame()
{
    if (FrameBuffer)
    {
        ...
        {
            d_memcpy(FrameBuffer + s_PositionX + (i + s_PositionY) * s_ScreenWidth, DG_ScreenBuffer + i * DOOMGENERIC_RESX, DOOMGENERIC_RESX * 4);
        }
    }
